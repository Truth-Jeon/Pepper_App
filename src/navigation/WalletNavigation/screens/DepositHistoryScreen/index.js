...
export const DepositHistoryScreen = ({navigation, route}) => {
  ...
  const getHistoryList = async _page => {
    ...
    try {
      ...
      if (page === 1) {
        setHistoryRowData([...response.results]);
      } else {
        ...
        <Sv>
          ...
          <Sv>
            ...
            <Sv>
              ...
              <St s2 mt={8}>
                {moment(transactionModalData.created_at).format(
                  'YYYY-MM-DD HH:MM',
                )}
              </St>
            </Sv>
